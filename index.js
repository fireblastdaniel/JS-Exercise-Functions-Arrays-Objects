 ...
 * }
*/
function makePersonObject(input_id, input_name, input_email) {
  const output = {
    id: input_id,
    name: input_name,
    email: input_email
 ...
 * it will return `This is a Lincoln Navigator`.
*/
function getCarInfoById(inventory, _id) {
...
function getGermanCars(inventory) {
  let german = [];
  for(let i = 0; i < inventory.length; i++){
    if(inventory[i].car_make in {'Audi':true, 'Mercedes-Benz':true, 'Volkswagen':true, 'BMW':true})
...
function carMaker(_odometer) {
  const car = {
    odometer: _odometer,
    drive: function(distance) {this.odometer += distance; return this.odometer;}
  }
  return car;
}
